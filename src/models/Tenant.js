...
const Tenant = {
  ...
  delete: async (id, options = {}) => {
    ...
    try {
      ...
      // 6. Delete Services (Canteen)
      if (options.services || options.fullAccount) {
        await connection.query('DELETE FROM services WHERE tenant_id = ?', [id]);
      }

      // 7. Delete other related data (Tickets, Staff, Notifications)
      if (options.fullAccount) {
        await connection.query('DELETE FROM tickets WHERE tenant_id = ?', [id]);
